...
class Ghosts():
    ...
    def update_position(self, y, x, direction): #не изменяет атрибуты
        ...
        if self.field.matrix[new_y][new_x] == '█':  #если новые координаты - стена, возвращаем первоначальные 
    ...
    def move(self): #изменяет атрибуты
        ...
        if self.field.matrix[prev_y][prev_x] == '-':
            self.field.mapa.addch(   #стираем с предыдущей позиции
                prev_y, 
                prev_x, 
                clear
            )
    ...
    def move_random(self):
        ...
        forward_directions = self.forward_directions(cur_direction) #возможные направления вперед, после текущего направления
        possible_directions = []
        
        for i in forward_directions: #вычисляем, какие направления вперед возможны (если нет препятствий)
            if self.update_position(cur_y, cur_x, i) != (self.y, self.x): # вычисляем возможность шага !!!без изменения текущих данных!!!
