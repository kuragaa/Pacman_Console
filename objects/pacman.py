...
class Pacman:
    ...
    def move(self, direction):
        ...
        if self.field.matrix[self.y][self.x] == '█' or self.field.matrix[self.y][self.x] == '-':  #если новые координаты - стена, возвращаем первоначальные 
            self.y = self.prev_y
            self.x = self.prev_x
            #self.stopped = True
        else:
            self.draw(self.prev_y, self.prev_x, direction)        # если не стена -> нужно отрисовать в новых координатах и стереть из предыдущих
